...
$admin[period_type] = array("pg_insert_datetime" => "등록일자", "pg_update_datetime" => "수정일자");
$admin[keyword_type] = array("" => "전체","pg_code" => "코드", "pg_name" => "항목명", "pg_memo" => "내용");
...
if($mode=="list_update"){
	...
	for($i=0; $i<count($chk); $i++){
		$k = $_POST['chk'][$i];
		sql_query("update $table set pg_use='{$_POST['pg_use'][$k]}' where $id='{$_POST[$id][$k]}'");
	}
...
if($pg_use) $sql_search .= " and pg_use = '$pg_use'";
...
<tr>
	...
	<th>노출여부</th>
	<td><?=admin_radio($_GET,"pg_use","","","0")?></td>
</tr>
<tr>
	<th>기간검색</th>
	<td colspan="3">
...
for($i=0; $row=sql_fetch_array($result); $i++){
	$row["chk[]"] = $i;
	$row["{$id}[{$i}]"] = $row[$id];
	$row["pg_use[]"] = $row[pg_use];
	$admin["pg_use[]"] = $admin[pg_use];
?>
<tr>
	<td><?=admin_checkbox($row,"chk[]","chk")?><?=admin_hidden($row,"{$id}[{$i}]")?></td>
	<td><a href="<?=$nfor[path]?>/page.php?pg_code=<?=$row[pg_code]?>" target="_blank"><?=$row[pg_code]?></a></td>
	<td><?=$row[pg_name]?></td>
	<td><?=admin_select($row,"pg_use[]","width-80p")?></td>
	<td><?=substr($row[pg_insert_datetime],0,10)?></td>
