...
$admin[keyword_type] = array(""=>"전체","pu_subject" => "제목", "pu_memo"=>"내용");
$admin[pu_target] = array(""=>"선택","Google" => "구글","Apple" => "애플");
$admin[period_type] = array("pu_insert_datetime" => "등록일자", "pu_update_datetime" => "수정일자", "pu_send_datetime" => "발송일자");
...
if($mode=="send"){
	...
	sql_query("update $table set pu_send_datetime=NOW() where $id='{$$id}'");


	$data[subject] = $write[pu_subject];
	$data[title] = $write[pu_subject];
	$data[memo] = $write[pu_memo];

	if(trim($write[pu_url])){
		$data[url] = $write[pu_url];
	}

	if($write[pu_img]){
		$data[image] = "$nfor[url]/data/push/$write[pu_img]";
	}

	if($write[pu_target]=="Google"){
		$message_status = send_notification("/topics/android", $data, "android"); 
	} elseif($write[pu_target]=="Apple"){
		$message_status = send_notification("/topics/ios", $data, "ios");
	} else{
...
if($pu_target) $sql_search .= " and pu_target='$pu_target' ";
...
<tr>
	<th>발송대상</th>
	<td><?=admin_select($_GET,"pu_target","width-150p","","0")?></td>
</tr>
<tr>
	<th>검색어</th>
	<td>
...
<tr>
	...
	<td><?=admin_img("push",$row[pu_img],"","40")?></td>
	<td><?=$row[pu_target]?></td>
	<td><?=$row[pu_subject]?></td>
	<td><?=$row[pu_url]?></td>
	<td><?=substr($row[pu_insert_datetime],0,10)?></td>
	<td><?=$row[pu_send_datetime]?substr($row[pu_send_datetime],0,10):"발송대기"?></td>
