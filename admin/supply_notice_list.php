...
$admin[keyword_type] = array(""=>"전체","no_subject" => "제목", "no_memo"=>"내용");
$admin[no_use] = array("전체","노출","미노출");
$admin[period_type] = array("no_insert_datetime" => "등록일자", "no_update_datetime" => "수정일자");	
$admin[no_category][""] = "전체";
...
$qstr .= "&no_category=$no_category&no_use=$no_use";
...
if($mode=="list_update"){
	...
	for($i=0; $i<count($chk); $i++){
		$k = $_POST['chk'][$i];
		sql_query("update $table set no_use='{$_POST['no_use'][$k]}' where $id='{$_POST[$id][$k]}'");
	}
...
if(!$is_admin){
	$sql_search .= " and no_use='1'";
}

$all_count_sql = "select count(*) as cnt $sql_common $sql_search";

if($no_category) $sql_search .= " and no_category = '$no_category'";

if($no_use) $sql_search .= " and no_use = '$no_use'";
...
<tr>
	<th>분류</th> 
	<td colspan="3"><?=admin_select($_GET,"no_category","width-150p","","0")?></td>
</tr>
<tr>
	<th>검색어</th>
	<td <? if(!$is_admin){ ?>colspan="3"<? } ?>>
	...
	<? if($is_admin){ ?>
	<th>노출여부</th>
	<td><?=admin_radio($_GET,"no_use","","","0")?></td>
	<? } ?>
</tr>
<tr>
	<th>기간검색</th>
...
for($i=0; $row=sql_fetch_array($result); $i++){	
	...
	$row["no_use[]"] = $row[no_use];
	$admin["no_use[]"] = $admin[no_use];
?>
<tr>
	<td><?=admin_checkbox($row,"chk[]","chk")?><?=admin_hidden($row,"{$id}[{$i}]")?></td>
	<td><?=$row[no_category]?></td>
	<td class="textleft"><a href="supply_notice_view.php?no_id=<?=$row[no_id]?>"><?=$row[no_subject]?></a></td>
	<td><?=substr($row[no_insert_datetime],0,10)?></td>
	<td><?=number_format($row[no_hit])?></td>
	<? if($is_admin){ ?>
	<td><?=admin_select($row,"no_use[]","width-80p")?></td>
