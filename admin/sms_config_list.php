...
if($mode=="add_templt"){
	...
	$tpl_name = $write['sd_name'];
	$tpl_content = $write['sd_msg'];

	$return = kakao_alarm_template_insert($tpl_name, $tpl_content);

	if($return['code']=="0"){
		$sd_templt_code = $return['data']['templtCode'];
		sql_query("update $table set sd_templt_code='$sd_templt_code' where $id='{$id_value}'");
	}
...
if($mode=="list_update"){
	...
	for($i=0; $i<count($chk); $i++){
		$k = $_POST['chk'][$i];
		sql_query("update $table set sd_sms_use='{$sd_sms_use[$k]}' where $id='{$_POST[$id][$k]}'");
	}
	json_return("정상적으로 수정되었습니다","ok");
}

$sql_common = " from $table ";
$sql_search = " where sd_sms_show='1' ";

$all_count_sql = "select count(*) as cnt $sql_common $sql_search";

if($sd_sms_use) $sql_search .= " and sd_sms_use = '$sd_sms_use'";
...
?>
<?=admin_help("잔여포인트가 없는 경우 알림톡 또는 문자메시지가 발송되지 않습니다.<br>SMS 발송자의 발신번호는 전기통신망법의 발신번호 사전등록제에 따라 사전에 등록된 번호만 사용할 수 있습니다.","line50 notice_gray")?>
...
<tr>
	...
	<th>사용여부</th>
	<td><?=admin_radio($_GET,"sd_sms_use","","","0")?></td>
</tr>
<tr>
	<th>기간검색</th>
	<td colspan="3">
...
for($i=0; $row=sql_fetch_array($result); $i++){	
	$row["chk[]"] = $i;
	$row["{$id}[{$i}]"] = $row[$id];
	$row["sd_msg[]"] = $row['sd_msg'];
	$row["sd_sms_use[]"] = $row['sd_sms_use'];
?>
<tr>
	<td><?=admin_checkbox($row,"chk[]","chk")?><?=admin_hidden($row,"{$id}[{$i}]")?></td>
	<td><?=$row['sd_name']?></td>
	<td><?=$row['sd_code']?></td>
	<td><?=admin_text($row,"sd_msg[]","",$row['sd_templt_code']?"readonly":"")?></td>
	<td><?=admin_select($row,"sd_sms_use[]","width-80p")?></td>
	<td><?=substr($row['sd_update_datetime'],0,10)?></td>
	<td><?=admin_a("edit", "수정", "btn btn-white btn-sm", "", "{$form}?{$qstr}&{$id}={$row[$id]}")?></td>
	<td>
		<?php if($row['sd_templt_code']){ ?>
			<?=$row['sd_templt_code']?>
		<?php } else{ ?>
			<?=admin_a("add_templt", "템플릿등록", "btn btn-white btn-sm nfor_button", "data-confirm=\"템플릿등록을 진행하시겠습니까?\" data-data=\"mode=add_templt&{$id}={$row[$id]}\"")?>
		<?php } ?>
	</td>
</tr>
<?php
...
<div class="bottom_btn">
	<div class="form-inline"><?=admin_button("list_update", "선택수정", "btn-lg btn-red btn")?></div>
</div>
