...
$admin[keyword_type] = array(""=>"전체", "fa_subject" => "제목", "fa_memo" => "내용");
$admin[fa_use] = array("전체", "노출", "미노출");
$admin[period_type] = array("fa_insert_datetime" => "등록일자", "fa_update_datetime" => "수정일자");
...
if($mode=="list_update"){
	...
	for($i=0; $i<count($chk); $i++){
		$k = $_POST['chk'][$i];
		sql_query("update $table set fa_rank='{$_POST['fa_rank'][$k]}', fa_use='{$_POST['fa_use'][$k]}' where $id='{$_POST[$id][$k]}'");
	}
...
if($fa_use) $sql_search .= " and fa_use = '$fa_use' ";
...
?>
<?=admin_help("사용자 페이지 고객센터 > 자주묻는질문에 게시글이 등록됩니다","line50 notice_gray")?>
...
<tr>
	<th>분류</th> 
	<td colspan="3"><?=admin_select($_GET,"fa_category","width-150p","","0")?></td>
</tr>
<tr>
	<th>검색어</th>
	<td>
	...
	<th>노출여부</th>
	<td><?=admin_radio($_GET,"fa_use","","","0")?></td>
</tr>
<tr>
	<th>기간검색</th>
	<td colspan="3">
...
for($i=0; $row=sql_fetch_array($result); $i++){
	...
	$row["fa_rank[]"] = $row[fa_rank];
	$row["fa_use[]"] = $row[fa_use];
	$admin["fa_use[]"] = $admin[fa_use];
?>
<tr>
	<td><?=admin_checkbox($row,"chk[]","chk")?><?=admin_hidden($row,"{$id}[{$i}]")?></td>
	<td><?=admin_echo($row,"fa_category")?></td>
	<td class="textleft"><?=$row[fa_subject]?></td>
	<td><?=admin_text($row,"fa_rank[]","width-50p")?></td>
	<td><?=admin_select($row,"fa_use[]","width-80p")?></td>
	<td><?=substr($row[fa_insert_datetime],0,10)?></td>
