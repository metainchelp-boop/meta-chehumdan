...
$admin[keyword_type] = array(""=>"전체","ag_code" => "코드", "ag_name"=>"항목명", "ag_memo"=>"내용");
$admin[ag_use] = array("전체","노출","미노출");
$admin[ag_group] = array("전체","개별","카피라이트");
$admin[period_type] = array("ag_insert_datetime" => "등록일자", "ag_update_datetime" => "수정일자");		
...
if($mode=="list_update"){
	...
	for($i=0; $i<count($chk); $i++){
		$k = $_POST['chk'][$i];
		sql_query("update $table set ag_use='{$_POST['ag_use'][$k]}' where $id='{$_POST[$id][$k]}'");
	}
...
if($ag_use) $sql_search .= " and ag_use='$ag_use' ";
...
<tr>
	...
	<th>노출여부</th>
	<td><?=admin_radio($_GET,"ag_use","","","0")?></td>
</tr>
<tr>
	<th>기간검색</th>
	<td colspan="3">
...
for($i=0; $row=sql_fetch_array($result); $i++){	
	$row["chk[]"] = $i;
	$row["{$id}[{$i}]"] = $row[$id];
	$row["ag_use[]"] = $row[ag_use];
	$admin["ag_use[]"] = $admin[ag_use];
?>
<tr>
	<td><?=admin_checkbox($row,"chk[]","chk")?><?=admin_hidden($row,"{$id}[{$i}]")?></td>
	<td><a href="<?=$nfor[path]?>/agreement.php?code=<?=$row[ag_code]?>" target="_blank"><?=$row[ag_code]?></a></td>
	<td class="textleft"><?=$row[ag_name]?></td>
	<td><?=admin_echo($row,"ag_group")?></td>
	<td><?=admin_select($row,"ag_use[]","width-80p")?></td>
	<td><?=substr($row[ag_insert_datetime],0,10)?></td>
