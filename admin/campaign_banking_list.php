...
$admin[keyword_type] = array("" => "전체", "bnk_bank" => "은행명", "bnk_number" => "계좌번호", "bnk_name" => "예금주");
$admin[bnk_use] = array("전체", "노출", "미노출");
$admin[period_type] = array("bnk_insert_datetime" => "등록일자", "bnk_update_datetime" => "수정일자");	
...
if($mode=="list_update"){
	...
	for($i=0; $i<count($chk); $i++){
		$k = $_POST['chk'][$i];
		sql_query("update $table set bnk_rank='{$_POST['bnk_rank'][$k]}', bnk_use='{$_POST['bnk_use'][$k]}' where $id='{$_POST[$id][$k]}'");
	}
...
if($bnk_use) $sql_search .= " and bnk_use = '$bnk_use'";
...
?>
<?=admin_help("사이트에서 무통장입금으로 주문 시 결제정보에서 선택하는 무통장입금 계좌정보를 관리합니다","line50 notice_gray")?>
...
<tr>
	...
	<th>노출여부</th>
	<td><?=admin_radio($_GET,"bnk_use","","","0")?></td>
</tr>
<tr>
	<th>기간검색</th>
	<td colspan="3">
...
for($i=0; $row=sql_fetch_array($result); $i++){
	...
	$row["bnk_rank[]"] = $row[bnk_rank];
	$row["bnk_use[]"] = $row[bnk_use];
	$admin["bnk_use[]"] = $admin[bnk_use];
?>
<tr>
	<td><?=admin_checkbox($row,"chk[]","chk")?><?=admin_hidden($row,"{$id}[{$i}]")?></td>
	<td><?=$row[bnk_bank]?></td>
	<td><?=$row[bnk_number]?></td>
	<td><?=$row[bnk_name]?></td>
	<td><?=admin_text($row,"bnk_rank[]","width-50p")?></td>
	<td><?=admin_select($row,"bnk_use[]","width-80p")?></td>
	<td><?=substr($row[bnk_insert_datetime],0,10)?></td>
