...
$admin['keyword_type'] = array(""=>"전체","cp_subject" => "제목", "cp_memo"=>"내용", "cp_name"=>"담당자", "cp_tel"=>"연락처", "cp_email"=>"이메일", "cp_company"=>"회사명", "cp_homepage"=>"홈페이지", "cp_mb_id"=>"아이디");
...
$admin['period_type'] = array("cp_insert_datetime" => "등록일자", "cp_reply_datetime" => "답변일자");	

$qstr .= "&cp_category=$cp_category&cp_reply_state=$cp_reply_state";
...
if($mode=="list_update"){
	...
	for($i=0; $i<count($chk); $i++){
		$k = $_POST['chk'][$i];
		if($_POST['cp_reply_state'][$k]=="2"){
			sql_query("update $table set cp_reply_state='{$_POST['cp_reply_state'][$k]}', cp_reply_datetime=NOW() where $id='{$_POST[$id][$k]}'");
		} else{
			sql_query("update $table set cp_reply_state='{$_POST['cp_reply_state'][$k]}', cp_reply_datetime='' where $id='{$_POST[$id][$k]}'");
		}
	}
...
if($cp_reply_state) $sql_search .= " and cp_reply_state = '$cp_reply_state' ";

if($cp_category) $sql_search .= " and cp_category = '$cp_category' ";
...
<tr>
	<th>분류</th> 
	<td colspan="3"><?=admin_select($_GET,"cp_category","width-150p","","0")?></td>
</tr>
<tr>
	<th>검색어</th>
	<td>
	...
	<th>답변상태</th> 
  	<td><?=admin_radio($_GET,"cp_reply_state","","","0")?></td>
</tr>
<tr>
	<th>기간검색</th>
	<td colspan="3">
...
for($i=0; $row=sql_fetch_array($result); $i++){	
	...
	$row["cp_reply_state[]"] = $row['cp_reply_state'];
	$admin["cp_reply_state[]"] = $admin['cp_reply_state'];
?>
<tr>
	<td><?=admin_checkbox($row,"chk[]","chk")?><?=admin_hidden($row,"{$id}[{$i}]")?></td>
	<td><?=$row['cp_category']?></td>
	<td class="textleft"><a href="<?="{$form}?{$qstr}&{$id}={$row[$id]}"?>"><?=$row['cp_subject']?></a></td>
	<td><?=$row['cp_company']?></td>
	<td><?=$row['cp_name']?><?=$row['cp_mb_id']?"<br>".$row['cp_mb_id']:""?></td>
	<td><?=$row['cp_tel']?></td>
	<td><?=substr($row['cp_insert_datetime'],0,10)?></td>
	<td>
	<?=admin_select($row,"cp_reply_state[]","width-100p")?>
	<?=$row['cp_reply_state']=="2"?substr($row['cp_reply_datetime'],0,10):""?>
	</td>
...
<div class="bottom_btn">
	...
	<?=admin_button("list_update", "선택수정", "btn btn-lg btn-black")?>
	</div>
