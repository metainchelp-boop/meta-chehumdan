...
$sql_search = " where rv_step='3' and rv_delete='0' ";
...
<tr>
	...
	<td><?php if($row[rv_buy_img]){ ?>
		<?php if($row[rv_buyer_name] && trim($row[rv_buyer_name])!=trim($row[rv_mb_name])){ ?><div style="color:#dc2626;font-size:11px;font-weight:bold;">⚠구매자:<?=$row[rv_buyer_name]?></div><?php } else if($row[rv_buyer_name]){ ?><div style="color:#0d9488;font-size:11px;">구매자:<?=$row[rv_buyer_name]?></div><?php } ?>
		<a href="<?=$nfor[path]?>/data/review/<?=$row[rv_buy_img]?>" target="_blank"><img src="<?=thumbnail($nfor[path]."/data/review/".$row[rv_buy_img],70,70,0,1)?>" style="border:1px solid #ddd;"></a><br>
		<a href="<?=$nfor[path]?>/data/review/<?=$row[rv_buy_img]?>" download class="btn btn-white btn-sm">다운로드</a>
	<?php } else{ ?>-<?php } ?></td>
