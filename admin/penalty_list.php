...
if($pe_mb_no) $sql_search .= " and pe_mb_no = '$pe_mb_no'";
...
<tr>
	...
	<th>구분</th>
	<td><?=admin_radio($_GET,"pe_type","","","0")?></td>
</tr>
<tr>
	<th>기간검색</th>
	<td colspan="3">
...
<tr>
	...
	<td><?=$row[pe_mb_id]?></td>
	<td><?=$row[pe_mb_nick]?></td>
	<td><a href="javascript:member('<?=$row[pe_mb_no]?>')"><?=$row[pe_mb_name]?></a></td>
	<td><?=$row[pe_mb_hp]?></td>
	<td><?=$row[pe_mb_email]?></td>
	<td><?=$row[pe_stop_memo]?></td>
	<td>
		<?=$admin[pe_type][$row[pe_type]]?>
		<? if($row[pe_type]=="3" or $row[pe_type]=="4"){ ?>
		<br><?=$row[pe_stop_day]?>일<br>(<?=date("Y년 m월 d일",strtotime($row[pe_mb_stop_date]))?>까지)
		<? } ?>
	</td>
	<td><?=$row[pe_datetime]?></td>
	<td><?=admin_a("pe_history", "이력보기", "btn btn-white btn-sm","","penalty_list.php?pe_mb_no=$row[pe_mb_no]")?></td>
</tr>
<?php
}
