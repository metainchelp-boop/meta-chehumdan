...
$admin[keyword_type] = array(""=>"전체","ke_keyword" => "키워드","ke_rank" => "순위","ke_current" => "변동");
$admin[period_type] = array("ke_insert_datetime" => "등록일자", "ke_update_datetime" => "수정일자");	
...
if($mode=="list_update"){
	...
	for($i=0; $i<count($chk); $i++){
		$k = $_POST['chk'][$i];
		sql_query("update $table set ke_rank='{$_POST['ke_rank'][$k]}',
										ke_keyword='{$_POST['ke_keyword'][$k]}',
										ke_current='{$_POST['ke_current'][$k]}'
										where ke_id='{$_POST['ke_id'][$k]}'");
	}
	keyword_update();
...
for($i=0; $row=sql_fetch_array($result); $i++){	
	...
	$row["ke_rank[$i]"] = $row[ke_rank];
	$row["ke_keyword[$i]"] = $row[ke_keyword];
	$row["ke_current[$i]"] = $row[ke_current];
?>
<tr>
	<td><?=admin_checkbox($row,"chk[]","chk")?><?=admin_hidden($row,"{$id}[{$i}]")?></td>
	<td><?=admin_text($row,"ke_rank[$i]")?></td>
	<td><?=admin_text($row,"ke_keyword[$i]")?></td>
	<td><?=admin_text($row,"ke_current[$i]")?></td>
	<td><?=substr($row[ke_insert_datetime],0,10)?></td>
