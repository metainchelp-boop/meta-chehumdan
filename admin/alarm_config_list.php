...
$admin[keyword_type] = array(""=>"전체","sd_name" => "구분", "sd_code"=>"코드", "sd_alarm_msg"=>"알림메시지내용");
...
if($mode=="list_update"){
	...
	for($i=0; $i<count($chk); $i++){
		$k = $_POST['chk'][$i];
		sql_query("update $table set sd_alarm_msg='{$sd_alarm_msg[$k]}', sd_alarm_use='{$sd_alarm_use[$k]}' where $id='{$_POST[$id][$k]}'");
	}
	json_return("정상적으로 수정되었습니다","ok");
}

$sql_common = " from $table ";
$sql_search = " where sd_alarm_show='1' ";

$all_count_sql = "select count(*) as cnt $sql_common $sql_search";



if($sd_alarm_use) $sql_search .= " and sd_alarm_use = '$sd_alarm_use'";
...
?>
<?=admin_help("알림은 사이트에서의 내부메시지로만 사용됩니다","line50 notice_gray")?>
...
<tr>
	...
	<th>사용여부</th>
	<td><?=admin_radio($_GET,"sd_alarm_use","","","0")?></td>
</tr>
<tr>
	<th>기간검색</th>
	<td colspan="3">
...
for($i=0; $row=sql_fetch_array($result); $i++){	
	$row["chk[]"] = $i;
	$row["{$id}[{$i}]"] = $row[$id];
	$row["sd_alarm_msg[]"] = $row[sd_alarm_msg];
	$row["sd_alarm_use[]"] = $row[sd_alarm_use];
?>
<tr>
	<td><?=admin_checkbox($row,"chk[]","chk")?><?=admin_hidden($row,"{$id}[{$i}]")?></td>
	<td><?=$row[sd_name]?></td>
	<td><?=$row[sd_code]?></td>
	<td><?=admin_text($row,"sd_alarm_msg[]")?></td>
	<td><?=admin_select($row,"sd_alarm_use[]","width-80p")?></td>
