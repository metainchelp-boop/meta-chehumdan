...
$admin[keyword_type] = array(""=>"전체","pop_subject" => "제목", "pop_memo"=>"내용");
...
$admin[period_type] = array("pop_insert_datetime" => "등록일자", "pop_update_datetime" => "수정일자");	

$qstr .= "&pop_device=$pop_device&pop_type=$pop_type&pop_use=$pop_use";
...
if($mode=="list_update"){
	...
	for($i=0; $i<count($chk); $i++){
		$k = $_POST['chk'][$i];
		sql_query("update $table set pop_use='{$_POST['pop_use'][$k]}' where $id='{$_POST[$id][$k]}'");
	}
...
if($pop_type) $sql_search .= " and pop_type='$pop_type' ";

if($pop_use) $sql_search .= " and pop_use='$pop_use' ";
...
<tr>
	<th>접근기기</th> 
	<td><?=admin_radio($_GET,"pop_device","","","0")?></td>
	<th>형태</th> 
	<td><?=admin_radio($_GET,"pop_type","","","0")?></td>
</tr>
<tr>
	<th>검색어</th>
	<td>
	...
	<th>노출여부</th> 
	<td><?=admin_radio($_GET,"pop_use","","","0")?></td>
</tr>
<tr>
	<th>기간검색</th>
	<td colspan="3">
...
for($i=0; $row=sql_fetch_array($result); $i++){	
	...
	$row["pop_use[]"] = $row[pop_use];
	$admin["pop_use[]"] = $admin[pop_use];
?>
<tr>
	<td><?=admin_checkbox($row,"chk[]","chk")?><?=admin_hidden($row,"{$id}[{$i}]")?></td>
	<td><?=admin_echo($row,"pop_device")?></td>
	<td><?=$row[pop_subject]?></td>
	<td><?=$row[pop_sdatetime]?> ~ <?=$row[pop_edatetime]?></td>
	<td><?=admin_select($row,"pop_use[]","width-80p")?></td>
