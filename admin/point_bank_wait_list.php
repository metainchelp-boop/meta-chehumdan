...
if($mode=="list_update1"){
	for($i=0; $i<count($chk); $i++){
		$k = $_POST['chk'][$i];
		sql_query("update nfor_point_bank set pb_step='3', pb_chage_datetime=NOW() where pb_id='{$_POST['pb_id'][$k]}'");
...
if($mode=="list_update2"){
	for($i=0; $i<count($chk); $i++){
		$k = $_POST['chk'][$i];
		sql_query("update nfor_point_bank set pb_step='4', pb_chage_datetime=NOW() where pb_id='{$_POST['pb_id'][$k]}'");
