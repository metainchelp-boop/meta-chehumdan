...
if($mode=="insert"){
	...
	if($member[mb_point] < $pb_point) json_return("출금신청 요청하신 포인트가 보유하신 포인트보다 적습니다","pb_point");
	...
	if(!$pb_jumin1) json_return("주민등록번호를 입력해주세요","pb_jumin1");
	if(!$pb_jumin2) json_return("주민등록번호를 입력해주세요","pb_jumin2");


	if(!$pb_filename1) json_return("신분증 사본을 첨부해주세요","pb_filename1");
	if(!$pb_filename2) json_return("통장 사본을 첨부해주세요","pb_filename2");

	$pb_jumin = $pb_jumin1."-".$pb_jumin2;

	insert_point($member[mb_no], $pb_point*-1, "출금신청", "700",$member[mb_no]."_".$ftimestamp);

	sql_query("insert nfor_point_bank set pb_filename1='$pb_filename1', pb_file1='$pb_file1', pb_filename2='$pb_filename2', pb_file2='$pb_file2', pb_jumin='$pb_jumin', pb_name='$pb_name', pb_bank='$pb_bank', pb_bank_number='$pb_bank_number', pb_point='$pb_point', pb_datetime=NOW(), pb_mb_no='$member[mb_no]', pb_mb_name='$member[mb_name]', pb_mb_id='$member[mb_id]', pb_step='1'");
