...
if($mode=="list_delete"){
	...
	for($i=0; $i<count($chk); $i++){
		$k = $_POST['chk'][$i];
		sql_query("update nfor_note set no_send_del='1' where no_id='{$_POST['no_id'][$k]}'");
	}
...
$sql_search = " where no_send_id='$member[mb_id]' and no_send_del='0' ";
...
for($i=0; $row=sql_fetch_array($result); $i++){
	...
	$res[no_receive_id] = $row[no_receive_id];
