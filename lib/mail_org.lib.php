...
function mailer($fname, $fmail, $to, $subject, $content, $type=0, $file="", $cc="", $bcc="") 
{
    ...
    $fname   = "=?$nfor[charset]?B?" . base64_encode($fname) . "?=";
    $subject = "=?$nfor[charset]?B?" . base64_encode($subject) . "?=";
	...
	$header .= "X-Mailer: nfor mailer 1.0 (nfor.net) : $_SERVER[SERVER_ADDR] : $_SERVER[REMOTE_ADDR] : $nfor[url] : $_SERVER[PHP_SELF] : $_SERVER[HTTP_REFERER] \n";

    if ($file != "") {
        $boundary = uniqid("http://nfor.net/");
    ...
    if ($type) {
        $header .= "Content-Type: TEXT/HTML; charset=$nfor[charset]\n";
        if ($type == 2)
            $content = nl2br($content);
    } else {
        $header .= "Content-Type: TEXT/PLAIN; charset=$nfor[charset]\n";
