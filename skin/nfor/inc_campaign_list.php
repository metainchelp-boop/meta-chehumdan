...
$(document).on("click",".sort_items_sns",function(){
	if($(".sort_div_sns").css("display") == "none"){		
		$(".sort_items_sns").addClass("on");
		$(".sort_div_sns").show();		
	} else{
		$(".sort_items_sns").removeClass("on");
		$(".sort_div_sns").hide();
	}
});
	...
	<div class="box">
			...
			<div class="it_info">
				<div class="top_info">
					...
					<?php if($row[cp_media_receipt]){ ?><span class="cp_media_receipt">영수증</span><?php } ?>
				...
				<div class="option2">
					...
					<a href="url_input_win.php?rv_id=<%=rv_id%>" class="url_input">리뷰URL<?=basename($PHP_SELF)=="review_asign_list.php"?"등록":"수정"?></a>
					<?php } ?>
				</div>
