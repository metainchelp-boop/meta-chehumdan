...
<div class="wrap_qna_view">
	...
	<ul class="qna_view_title">
		...
		<p class="wr_it_name"><? if($qna[it_name]){ ?><a href="campaign.php?cp_id=<?=$qna[cp_id]?>"><?=$qna[it_name]?></a><? } else{ ?>선택한 캠페인이 없습니다.<? } ?></p>
	...
	<ul class="qna_view_qna">
		...
		<?
		$que2 = sql_query("select * from nfor_campaign_qna where wr_reply='$qna[wr_id]' order by wr_id desc");
	...
	<a href="my_campaign_qna_list.php"  class="btn_list" style="display: block; width: 161px; height: 46px; background: #383838;  color: #fff; font-weight: normal;border: 0; line-height:46px; margin:20px auto; font-size: 18px; text-align:center;">목록보기</a>
