...
<div class="board_btn_zone" style="text-align:right">
	<span class="btn_pack"><a href="board_list.php?tbl=<?=$board[bo_tbl]?>&nf_category=<?=$nf_category?>&page=<?=$page?>" class="btn_md black">목록보기</a></span>
	<?php if($member[mb_no] and ($is_admin or $write[nf_mb_no]==$member[mb_no])){ ?>
	<span class="btn_pack"><a href="board_form.php?tbl=<?=$board[bo_tbl]?>&nf_id=<?=$write[nf_id]?>" class="btn_md white">수정하기</a></span>
	<span class="btn_pack"><a class="btn_md white del_btn" data-nf_id="<?=$write[nf_id]?>">삭제하기</a></span>
	<?php } ?>
</div>
