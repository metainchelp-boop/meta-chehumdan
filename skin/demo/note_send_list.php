...
<div class="note_receive_top">
	<span>보낸쪽지 <b class="txt_num point_color3"><?=$total_count?></b>통</span>
</div>
...
<form name="flist" id="flist" method="post">
	...
	<div class="data_table thead_colored">
		...
		<tr>
			...
			<td class="txt_num date"><?=$row[no_receive_id]?></td>
			<td class="left_txt"><a href="note_send_view.php?no_id=<?=$row[no_id]?>" ><?=$row[no_memo]?></a></td>
			<td><a href="note_send_view.php?no_id=<?=$row[no_id]?>" class="txt_num date"><?=$row[no_send_datetime]?></a></td>
			<td><a href="note_send_view.php?no_id=<?=$row[no_id]?>" class="txt_num date"><?=$row[no_receive_datetime]?></a></td>
		</tr>
		<?php
		} 
		if(!$i){
