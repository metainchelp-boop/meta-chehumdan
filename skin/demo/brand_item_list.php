...
<div class="item_list">
	...
	<div class="subpath">	
		...
		echo "<a href=\"$PHP_SELF?it_bcategory_id=".$navi[category_id][0]."\">".$navi[cg_category][0]."</a>";

		for($i=1; $i < count($navi[category_id]); $i++){
			echo "<img src=\"".$nfor[skin_path]."img/arrow.png\">"; 
			echo "<a href=\"$PHP_SELF?it_bcategory_id=".$navi[category_id][$i]."\">".$navi[cg_category][$i]."</a>";
		}
		?>
	</div>
	...
	<div class="top_cate_menu2">
		...
		<?
		$cg_depth = strlen($it_bcategory_id)/3;
		$sql = "select * from nfor_item_brand where cg_use='1' and cg_depth='$cg_depth'";
		if($it_bcategory_id) $sql .= " and category_id like '$it_bcategory_id%'";
		$sql .= " order by cg_rank asc";
		$que = sql_query($sql);
		if(sql_num_rows($que)){
		?>
		<ul class="navi">
			<? if(substr($it_bcategory_id,0,-3)){ ?><li class="<?=$it_bcategory_id?"on":""?>"><a href="<?=$PHP_SELF?>?it_bcategory_id=<?=$it_bcategory_id?>"><span class="txt">전체보기</span></a></li><? } ?>
			<?
			while($data = sql_fetch_array($que)){
			?>
			<li class="<?=$data[category_id]==$it_bcategory_id?"on":""?>"><a href="<?=$PHP_SELF?>?it_bcategory_id=<?=$data[category_id]?>"><span class="txt"><?=$data[cg_category]?></span></a></li>
			<? } ?>
		</ul>
		<? } ?>
...
$(document).on("click", ".select_orderby a", function(){
	location.href="<?="$PHP_SELF?it_bcategory_id=$it_bcategory_id&orderby="?>" + $(this).data("orderby");	
});
</script>


<?=$brand[cg_category_tail]?>
